...
5. **Next move** — named and then *done*: another variation, a doc revision, a refs/media swap (pull different media from `04 media/` or different refs), an asset that must be made first, or — only when the doc genuinely cannot answer — a flagged question for the principal that the run routes around, never waits on.
