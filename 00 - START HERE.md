...
2. **Work from a content concept, not a blank page.** `06 content concepts/` holds the decided pieces — each one says what the piece is for, who it moves, the one message, and the angle. If a concept doesn't exist yet for what you want to make, it gets written first (step L1 in `function/steps/`).

3. **Build the brief.** Per concept, walk the steps in `function/steps/`: L2 outline (what it covers, in what order, the single point), L3 visual direction (which expressions are in play, which media lands where — this is where you pull the expression docs and the media CDN docs), L4 copy (good enough for the round's mode — see the bar below). Each step doc says exactly what to read and where to write. The result accumulates in `07 briefs/[concept]/` in the template's shape.
...
5. **Capture at the moment of making.** Before moving to the next thing: screenshot/export the result and write the small markdown per `08 output/_contract.md` — which tool, which prompt, which expressions were in play, and your honest commentary. Then copy that capture into each source expression's `explorations/`. Feedback captured in the moment is what the next round runs on; feedback left in your head is gone.
...
- Facts come from `05 knowledge/` only: no invented facts, nothing planned framed as already real.
