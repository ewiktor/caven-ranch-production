...
- **Locked / filled / cycled controls** (DRAPER fill-states, per the schema board) — the governing mechanic. A production type has a full control list; each control ends up in one of three states: **locked** — the user's commentary spoke to it, so it's fixed as keywords/terms/phrases (typically only a few of the full set); **filled** — a subject-matter control, determined by the actual `06 knowledge/` and `03 media/`, not free to invent; **cycled** — genuinely open, and cycling these is what makes the variations. Two hard rules: a lock is never absolute or one-directional unless the user's words were — hedges stay hedged, and negatives record only things the user killed. And filling/cycling a non-locked control follows this order: the references this expression tags (in `04 references/`) → the user's documented taste across the other expression docs (docs cite each other as evidence freely) → 01 settled + 06 knowledge → the user's live direction in session, which supersedes stale doc text and must be written back the same session, version-bumped → the agent's own judgment last, flagged as such.
