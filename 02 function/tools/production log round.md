...
Review one expression's just-finished variation set and act on what the review finds. The per-derivation capture (`10 output/_contract.md`) records what was made; the round is the QA — the system checks itself, nothing waits on a human. A round that only logs has failed.
...
Loop depth follows the check tiering in `00 index.md` (the mode line in `00 brand.md` says which mode the current round runs in). **High-volume rounds: one round per variation set by default** — re-enter the build-again loop only when the round finds a doc or prompt fault worth rebuilding for; otherwise record, deliver per your run plan, move to the next expression. **Per-expression budget in a volume pass: the variation budget in `00 brand.md`, then log and move on** — later passes return to the expression with the prior round's preserve/avoid in hand. Never declare an axis "exhausted" by feel; axes are combinatorial and a full sweep is not the job. Iterate-until-converged is slow-production behavior, not a volume-pass gate — a run that perfects one expression and never reaches the rest has failed the round it's in.
...
- **Generation lane** (a compiled prompt from `09 briefs/` runs in a tool): doc / prompt / execution / axis-finding.
...
**Reads:** the set's capture.md files · the expression's `doc.md` (the control set being tested) and `feedback.md` · `refs/` for the judging standard · `01 logic/controls - composition.md` / `controls - graphic.md` and the matching AD logic block, when classifying a finding against controls · prior rounds in the expression's `explorations/`
**Writes:** the round entry is **one markdown file in the expression's `explorations/` folder, named `round-[tool]-[date].md`** (second round same tool same day: `-2`). That is the primary home in every lane, always — no deciding, no stalling. If the set ran under a concept, copy the round file into `10 output/[concept]/` alongside the run record (same copy-back symmetry as the contract); if there is no concept run (expression studies, asset efforts), `explorations/` is the whole record and `10 output/` gets nothing. Also writes `doc.md` revisions when the fault line lands there — version-bumped, marked "(agent call, review)".
...
5. **Next move** — named and then *done*: another variation, a doc revision, a refs/media swap (pull different media from `05 media/` or different refs), an asset that must be made first, or — only when the doc genuinely cannot answer — a flagged question for the principal that the run routes around, never waits on.
