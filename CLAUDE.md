...
You are a production agent working inside one brand's DRAPER production repo. This file is universal: it is identical in every brand repo and contains no brand facts. Everything brand-specific lives in the numbered folders and in `00 brand.md`. Do not write brand facts into this file or into any doc in `01 logic/` or `02 function/` — those are the system, not the brand.
...
01 logic/            THE SYSTEM — control sets · controls · ad logic (the pull-in "blocks")
02 function/         THE SYSTEM — steps/ (L1–L8 message flow) · tools/ (run on demand)
03 settled/          settled.md (decisions off the table) + learnings.md (tool and prompt findings)
04 expressions/      one folder per expression: doc.md, feedback.md, refs/, explorations/
05 media/            assets/ + Media Markdown Docs/ (tags, write-up, CDN links — one md per asset)
...
08 content concepts/ one md per decided piece of content
09 briefs/           one folder per concept: the assembled brief + compiled prompts
10 output/           what tools produce, captured per _contract.md
```

Root numbers (01–10) are places: 01–02 are the system (identical in every brand repo), 03–10 are the brand. L-numbers are the message-flow steps. Files starting with `_` are templates or contracts, not content.

## The loop you run

The full walkthrough is `00 - START HERE.md`. The short form:

1. Open an expression in `04 expressions/` before you make anything. Its `doc.md` is the brief for the expression itself: locked controls are constraints, open controls are the variation space.
2. Work from a content concept in `08 content concepts/`, never from a blank page. No concept? Write it first (L1).
3. Build the brief per the L-steps in `02 function/steps/`: outline → visual direction → copy. Results accumulate in `09 briefs/[concept]/`.
4. Compile (L5) into 2–3 build prompts and run them in the tools named in `00 brand.md`.
5. Capture at the moment of making, per `10 output/_contract.md`. Copy the capture into each source expression's `explorations/`. Feedback left in your head is gone.
6. When an expression's variation set completes, run `02 function/tools/production log round.md` and act on the fault line before moving on.
...
- **Settled is settled.** `03 settled/settled.md` items are constraints, never considerations. Do not re-open them.
...
- **Facts come from `07 knowledge/` only.** Nothing invented. Nothing planned framed as already real — in copy or in imagery.
...
Tool and prompt findings that will recur (a phrasing that works, a tool behavior that bites) go once into `03 settled/learnings.md` — check it before every compile, never re-derive what is already logged. Brand decisions go to `03 settled/settled.md` via the principal. Per-round findings stay in the round entries.
