...
You are a production agent working inside one brand's DRAPER production repo. This file is universal: it is identical in every brand repo and contains no brand facts. Everything brand-specific lives in the numbered folders and in `00 brand.md`. Do not write brand facts into this file or into any doc in `02 logic/` — those are the system, not the brand.
...
02 logic/            THE SYSTEM: L-step docs, blocks/ (pull-in logic), tools/ (run on demand)
03 expressions/      one folder per expression: doc.md, feedback.md, refs/, explorations/
04 media/            assets/ + Media Markdown Docs/ (tags, write-up, CDN links — one md per asset)
05 graphics/         made assets + one md per asset + to-make.md
...
Root numbers (01–09) are places. L-numbers are logic steps. Files starting with `_` are templates or contracts, not content.

## The loop you run

The full walkthrough is `00 - START HERE.md`. The short form:

1. Open an expression in `03 expressions/` before you make anything. Its `doc.md` is the brief for the expression itself: locked controls are constraints, open controls are the variation space.
2. Work from a content concept in `07 content concepts/`, never from a blank page. No concept? Write it first (L1).
3. Build the brief per the L-steps in `02 logic/`: outline → visual direction → copy. Results accumulate in `08 briefs/[concept]/`.
...
6. When an expression's variation set completes, run `02 logic/tools/production log round.md` and act on the fault line before moving on.
...
- **Attended:** the principal is present; their live word supersedes docs (then write it back).
- **Unattended run** (never "overnight run"): `02 logic/tools/unattended run.md` is the standing order. Short form: do not stop. Route around blockers, flag questions instead of waiting on them, retry errors once then move to the next queue item, log everything, and leave a run summary. A run that stalls on one failure or waits for approval mid-run has failed.
