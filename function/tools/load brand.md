...
references/
  ...
  context.md                          per-reference PRINCIPAL COMMENTARY + CDN  ← the load-bearing part
explorations/
...
1. **Read the manifest** (`brief.md`) to see the full inventory. Read each folder's `context.md` — that's where the principal's commentary lives.
...
3. **Fill `00 brand.md` from the strategy overview** — the brand, what's being made, the one-line. **Do not invent** the principal's name, the toolchain, or the round/mode — those aren't in the export. Leave them as `[TODO — ask the human]` and list them in your report. An unfilled `00 brand.md` is what tells a later agent to stop, so getting it filled (with the human's help) is required before production.
...
7. **Explorations → `input/explorations/`.** Copy each md + asset. These are prior outputs (each has the principal's commentary + the prompt that made it). They are pre-board-pass — do NOT turn them into expressions. They stage here until the board pass groups them.
...
9. **Report.** Write a short load report to the chat:
   ...
   - what's still needed in `00 brand.md` (principal, toolchain, round + mode) — ask the human for these,
